...
export const Route = createFileRoute("/assistant")({
  head: () => ({
    meta: [
      { title: "AI Assistant — VerdantAI" },
      { name: "description", content: "Ask questions about your crops, diseases, or farming practices." },
    ],
  }),
      ...
      <div className="grid lg:grid-cols-4 gap-6 h-[calc(100vh-13rem)]">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-3 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                <div className={`h-9 w-9 rounded-full shrink-0 flex items-center justify-center ${m.role === "user" ? "bg-terracotta text-terracotta-foreground" : "bg-gradient-leaf text-leaf-foreground"}`}>
                  {m.role === "user" ? "RA" : <Leaf className="h-4 w-4" />}
                </div>
                <div className={`max-w-xl rounded-2xl px-4 py-3 text-sm leading-relaxed ${m.role === "user" ? "bg-terracotta text-terracotta-foreground" : "bg-secondary text-foreground"}`}>
                  {m.content}
                </div>
              </div>
            ))}
        ...
        <Card className="p-5 border-border shadow-soft h-fit">
          ...
          <div className="space-y-2">
            {suggestions.map((s) => (
              <button key={s} onClick={() => send(s)} className="w-full text-left text-sm p-3 rounded-lg bg-secondary/50 hover:bg-secondary border border-border hover:border-leaf/40 transition">
                {s}
              </button>
            ))}
          </div>
