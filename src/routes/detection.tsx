...
function DetectionPage() {
  ...
  return (
    <AppShell title="Famantarana Aretina" subtitle="Ampidiro ny sarin'ny ravina. Hamantatra ny aretina ny modely AI ary hanazava ny antony.">
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <Card className="lg:col-span-2 p-6 border-border shadow-soft">
          ...
          <div className="aspect-square rounded-2xl border-2 border-dashed border-border bg-secondary/40 flex flex-col items-center justify-center relative overflow-hidden group">
            {analyzed || loading ? (
              <>
                ...
                {loading && (
                  <div className="absolute inset-0 bg-ink/40 backdrop-blur-sm flex flex-col items-center justify-center">
                    ...
                    <p className="text-primary-foreground mt-4 font-display italic">Mamaky ny ravina…</p>
                  </div>
                )}
              </>
            ) : (
              <>
                ...
                <p className="font-medium">Avariano sary eto</p>
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            <Button variant="outline" className="gap-2"><Camera className="h-4 w-4" /> Fakan-tsary</Button>
            <Button variant="outline" className="gap-2" onClick={() => { setAnalyzed(false); setLoading(false); }}>Avereno</Button>
          </div>
          ...
          <Button className="w-full mt-3 bg-leaf hover:bg-leaf/90 text-leaf-foreground gap-2" size="lg" onClick={analyze} disabled={loading}>
            <Sparkles className="h-4 w-4" /> {loading ? "Manadihady…" : "Analyze with AI"}
          </Button>
        </Card>
        ...
        <div className="lg:col-span-3 space-y-6">
          {!analyzed ? (
            <Card className="p-12 border-dashed border-border shadow-soft flex flex-col items-center justify-center text-center min-h-[400px]">
              ...
              <p className="text-muted-foreground mt-2 max-w-sm">Hiseho eto ny famantarana miaraka amin'ny drafi-pitsaboana sy fiarovana avy amin'ny boky momba ny fambolena.</p>
            </Card>
          ) : (
            <>
              <Card className="p-6 border-border shadow-elegant">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <Badge className="bg-destructive/10 text-destructive border-0 mb-3">Hamafiny avo</Badge>
                  ...
                  <div className="text-right">
                    <div className="text-xs uppercase tracking-wider text-muted-foreground">Fahatokisana</div>
