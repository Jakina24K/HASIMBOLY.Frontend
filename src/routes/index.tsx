...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Tabilao fanaraha-maso — HASIMBOLY.AI" },
      {
        name: "description",
        content:
          "Fikarohana aretin’ny zavamaniry sy fanolorana voly marani-tsaina ho an’ny tantsaha eto Madagasikara.",
      },
    ],
  }),
  component: Dashboard,
...
function Dashboard() {
  ...
  return (
    <AppShell
      title="Manao ahoana, Rasoa"
      subtitle="Inty ny zavatra lazain'ny tanimbolinao androany."
    >
      {/* Hero */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-hero text-primary-foreground mb-8 shadow-elegant">
        ...
        <div className="relative px-8 lg:px-12 py-12 lg:py-16 max-w-2xl">
          ...
          <h2 className="font-display text-4xl lg:text-5xl mt-4 leading-tight text-balance">
            Tanimboly roa mila fitandremana.{" "}
            <span className="italic text-leaf">Telo no mandroso tsara.</span>
          </h2>
          <p className="mt-4 text-primary-foreground/70 max-w-lg">
            Ny fitambaran'ny ML diagnostics sy ny semantic reasoning no manasongadina izay
            zava-dehibe eo amin'ny tanimbolinao rehetra.
          </p>
          <div className="flex gap-3 mt-8">
            <Button asChild size="lg" className="bg-leaf hover:bg-leaf/90 text-leaf-foreground">
              <Link to="/detection">Fantaro ny aretin’ny ravina</Link>
            </Button>
            ...
            >
              <Link to="/recommendation">Makà tolo-kevitra</Link>
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <Stat
          icon={Microscope}
          label="Famantarana tamin'ity herinandro ity"
          value="128"
          delta="12%"
          tone="leaf"
        />
        ...
        <Stat
          icon={Activity}
          label="Salan'isan'ny fahatokisana"
          value="91%"
          delta="2.4%"
          tone="leaf"
        />
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Famakafakana dimy farany teny an-tsaha
              </p>
            </div>
            <Link
              to="/history"
              className="text-sm text-leaf hover:underline inline-flex items-center gap-1"
            >
              ...
              <div key={d.id} className="py-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-sm">
                    {d.crop} —{" "}
                    <span className={d.disease === "Healthy" ? "text-success" : "text-terracotta"}>
                      {d.disease}
                    </span>
                  </div>
        ...
        <Card className="p-6 border-border shadow-soft">
          ...
          <p className="text-sm text-muted-foreground mb-5">
            Avy amin'ny toetrandro sy ny ontology
          </p>
            ...
            {alerts.map((a) => (
              <div key={a.id} className="p-4 rounded-xl border border-border bg-secondary/40">
                <div className="flex items-start gap-3">
                  <span
                    className={`mt-1 h-2 w-2 rounded-full shrink-0 ${a.level === "high" ? "bg-destructive" : a.level === "medium" ? "bg-warning" : "bg-leaf"}`}
                  />
        ...
        <Card className="p-6 border-border shadow-soft lg:col-span-2">
            ...
            {usageStats.map((s) => (
              <div key={s.day} className="flex-1 flex flex-col items-center gap-2">
                <div className="w-full flex items-end gap-1 h-40">
                  <div
                    className="flex-1 bg-leaf/80 rounded-t-md hover:bg-leaf transition"
                    style={{ height: `${(s.diagnoses / max) * 100}%` }}
                  />
                  <div
                    className="flex-1 bg-terracotta/70 rounded-t-md hover:bg-terracotta transition"
                    style={{ height: `${(s.recommendations / max) * 100}%` }}
                  />
                </div>
          ...
          <div className="flex gap-4 mt-4 text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-sm bg-leaf" /> Famantarana
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-sm bg-terracotta" /> Tolo-kevitra
            </span>
          </div>
        </Card>
